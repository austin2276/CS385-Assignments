...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        z->left = NULL;     // z's left child is NULL
        z->right = NULL;    // z's right child is NULL
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // TODO 2 - DONE
        // Implements a recursive check to delete all subtrees
        if (n == NULL) {       // Base Case
            delete n;          // If the tree is empty, delete
        }
        else {                 // If its children aren't empty, go to each non-empty child and recurse
            if (n->left != NULL) {
                delete_tree(n->left);
            }
            if (n->right != NULL) {
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        Node<K,V> *y;
        size_++;
        while ((z->parent != NULL) && (z->parent->color == RED)) {   // _ && _
            if (z->parent == z->parent->parent->left) {             // _
                y = z->parent->parent->right;                       // c
                if ((y != NULL) && (y->color == RED)) {             // _ && _
                    z->parent->color = BLACK;                       // c
                    y->color = BLACK;                               // c
                    z->parent->parent->color = RED;                 // c
                    z = z->parent->parent;                          // c
                }
                else {
                    if (z == z->parent->right) {                    // _
                        z = z->parent;                              // c
                        left_rotate(z);                             // c
                    }
                    z->parent->color = BLACK;                       // c
                    z->parent->parent->color = RED;                 // c
                    right_rotate(z->parent->parent);                // c
                }
            }
            else {
                y = z->parent->parent->left;                        // c
                if ((y != NULL) && (y->color == RED)) {             // _ && _
                    z->parent->color = BLACK;                       // c
                    y->color = BLACK;                               // c
                    z->parent->parent->color = RED;                 // c
                    ...
                    if (z == z->parent->left) {                     // _
                        z = z->parent;                              // c
                        right_rotate(z);                            // c
                    }
                    z->parent->color = BLACK;                       // c
                    if (z->parent->parent != NULL) {                // _
                        z->parent->parent->color = RED;             // c
                        left_rotate(z->parent->parent);             // c
                    }
                }
            }
        }
    ...
    size_t leaf_count(Node<K, V> *node) const {
        // TODO 7 - DONE
        size_t countOfLeaves = 0;
        if (node == NULL) {
            return 0;
        }
        if (node->left != NULL) {
            countOfLeaves += leaf_count(node->left);
        }
        if (node->right != NULL) {
            countOfLeaves += leaf_count(node->right);
        }
    ...
    int diameter(Node<K, V> *node) const {
		...
		return std::max(lh+rh+2, std::max(ld,rd));
        // END 9
    }
